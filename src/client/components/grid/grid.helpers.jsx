...
export function generateNodeCoordinates(requirements) {
    ...
    function generateCoordinate(range) { // get randomised coordinate array
        const { maxRows, maxColumns } = range;
        function getRandomIntMinMax(min, max) {
            min = Math.ceil(min);
            max = Math.floor(max);
            return Math.floor(Math.random() * (max - min)) + min; //The maximum is exclusive and the minimum is inclusive
        }
        const coordinates = [getRandomIntMinMax(0, maxRows), getRandomIntMinMax(0, maxColumns)]; // amount of rows - columns
        return coordinates;
    }
    function checkUniqueValues(coordinateArrayCheck) { //
        ...
        if (uniqueCoordinates.length === amountOfNodes.length) {
            ...
            // generate coordinate
            const extraCoordinates = generateCoordinate(maxRange);
            // push into array
